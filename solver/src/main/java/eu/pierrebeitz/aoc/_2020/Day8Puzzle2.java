...
public class Day8Puzzle2 {
    ...
    private static class RuntimeExecutor extends RecursiveTask<Integer> {
        ...
        // this approach actually allows to solve a more complex version of this problem where we would mutate any
        // instruction
        // and not only one
        @Override
        protected Integer compute() {
            ...
            while (runtime.hasNext()) {
                ...
                runtime.executeNext();
                sum += maybeMutatedRuntime.map(RuntimeExecutor::new).map(RecursiveTask::fork).stream()
